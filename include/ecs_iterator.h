...
namespace ecs {
	...
	template <typename T>
	struct IsComponent
		: std::integral_constant<bool, std::is_base_of<BaseComponent, T>::value && !std::is_same<BaseComponent, T>::
		                         value> {};
	...
	template <typename T>
	struct IsComponentPointer
		: std::integral_constant<bool, IsComponent_v<std::remove_pointer<T>> && !std::is_pointer<T>::value> {};
	...
	private:
		...
		private:
			...
			auto GetLeastComponentVector(EntityAdmin* admin,
			                             ComponentVector& compare_vector) -> std::enable_if_t<
				(sizeof...(Us) != 0), ComponentVector&>
			{
				auto& component_vector = admin->GetAllComponents(details::ComponentIndex::index<U>());
				return GetLeastComponentVector<Us...>(
					admin, compare_vector.size() <= component_vector.size() ? compare_vector : component_vector);
			}
